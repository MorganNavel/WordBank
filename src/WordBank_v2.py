...
def create_table():
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS "Translation" ("French" TEXT NOT NULL,"Korean" TEXT NOT NULL,"English" TEXT NOT NULL,"ID" INTEGER,PRIMARY KEY("ID"))')
    connection.commit()
    connection.close()
...
def translation_english_to_french_korean(english):
    ...
    result="French: "+cursor.fetchone()[0]+"\n"
...
def translation_french_to_english_korean(french):
    connection_cursor=connection_data_base("base.db")
    connection=connection_cursor[0]
    cursor=connection_cursor[1]
    show_translation_form(window)
...
def translation_korean_to_english_french(korean):
    connection_cursor=connection_data_base("base.db")
    connection=connection_cursor[0]
    cursor=connection_cursor[1]
    show_translation_form(window)
...
def translate_word(english,french,korean):  
    if(french!="" and english=="" and korean==""):
        translation_french_to_english_korean(french)
    elif(french!="" and (english!="" or korean!="")):
        send_error_too_much_field_fill()
    elif(english!="" and french=="" and korean==""):
        translation_english_to_french_korean(english)
    elif(english!="" and (french!="" or korean!="")):
        send_error_too_much_field_fill()       
    elif(korean!="" and french=="" and english==""):
        translation_korean_to_english_french(korean)      
    elif(korean!="" and (french!="" or english!="")):
        error_too_much_field_fill()
    elif(english=="" and korean=="" and french==""):
...
def register_word(english,french,korean):
    connection_cursor=connection_data_base("base.db")
    connection=connection_cursor[0]
    cursor=connection_cursor[1]
    if(korean!="" and english!="" and french!=""):
        new_word=(french,korean,english,cursor.lastrowid)
...
def saved_words(window):
    ...
    connection=connection_cursor[0]
    cursor=connection_cursor[1]
    cursor.execute("SELECT English,French,Korean FROM Translation")
...
def create_form(window):
    label_title = Label(window, text="Translation App",style="Custom.TLabel").grid(row=0,column=0)
    english=show_english_part_form("Enter an English word")
    french=show_french_part_form("Enter a French word")
    korean=show_korean_part_form("Enter a Korean word")
    return [english,french,korean]
...
def show_french_part_form(text):
    french_word=StringVar()
    label_french = Label(window, text="Enter French word",style="Label").grid(row=2,column=0,pady=10)
    french_field = Entry(window,text="French",textvariable = french_word).grid(row=2,column=1,pady=10)
    return french_word
    
    
def show_korean_part_form(text):
    korean_word=StringVar()
    label_korean = Label(window, text="Enter Korean word",style="Label").grid(row=3,column=0)
    korean_field = Entry(window,text="Korean",textvariable=korean_word).grid(row=3,column=1)
    return korean_word
...
def show_practice(back):
    ...
    Button(window,style="Custom.TButton",text="French",command=lambda:show_practice_form("French")).grid(row=2,pady=5)
...
def show_register_form(window):
    ...
    button_submit = Button(window,style="Custom.TButton",text="Register",command=lambda:register_word(entry_values[0].get(),entry_values[1].get(),entry_values[2].get())).grid(row=4,column=0,pady=5)
...
def show_translation_form(window):
    refresh_page(window)
    entry_values=create_form(window) 
    button_submit = Button(window,style="Custom.TButton",text="Translate",command=lambda:translate_word(entry_values[0].get(),entry_values[1].get(),entry_values[2].get())).grid(row=4,column=0,pady=5)
...
def show_word_korean(words,cursor,connection,nbTry,nbEasy):
    refresh_page(window)
    cursor.execute("SELECT English,French FROM Translation WHERE ID=?",[str(words[0])])
    j=0
...
def show_word_english(words,cursor,connection,nbTry,nbEasy):
    refresh_page(window)
    cursor.execute("SELECT French,Korean FROM Translation WHERE ID=?",[str(words[0])])
    j=0
    for word in cursor.fetchone():
        if j==0:
            Label(window,text=word,style="Label").grid(row=0,column=j,padx=5)
...
def show_word_french(words,cursor,connection,nbTry,nbEasy):
    refresh_page(window)
    cursor.execute("SELECT English,Korean FROM Translation WHERE ID=?",[str(words[0])])
    j=0
    for word in cursor.fetchone():
        if j==0:
            Label(window,text=word,style="Label").grid(row=0,column=j,padx=5)
...
def addFirst(words,word,cursor,connection,language,nbTry,nbEasy):
    ...
    refresh_page(window)
    if(language=="French"):
        practice_french(window, cursor, connection, words, nbTry, nbEasy)
...
def show_hard_easy_response(words,cursor,connection,language,nbTry,nbEasy):
    if language=="Korean":
        ...
        show_word_french(words, cursor, connection,nbTry,nbEasy)
        Button(window,style="Custom.TButton",text="EASY",command=lambda:addLast(words, words[0],cursor,connection,"French",nbTry,nbEasy)).grid(row=2,column=0)
        Button(window,style="Custom.TButton",text="HARD",command=lambda:addFirst(words, words[0],cursor,connection,"French",nbTry,nbEasy)).grid(row=2,column=3)    

def init_practice_french(window):
    db_path="base.db"
    connection_cursor=connection_data_base(db_path)
    connection=connection_cursor[0]
    cursor=connection_cursor[1]
    ...
    refresh_page(window)
    cursor.execute("SELECT French FROM Translation WHERE ID=?",[str(words[0])])
    Label(window,text=cursor.fetchone(),style="Label").grid(row=1)
    nbTry=0
    nbEasy=0
    Button(window,style="Custom.TButton",text="NEXT",command=lambda:show_hard_easy_response(words,cursor,connection,"French",nbTry,nbEasy)).grid(row=2)
    
    
def practice_french(window,cursor,connection,words,nbTry,nbEasy):
    refresh_page(window)
    if(words==[]):
...
style.configure("Label",background="white",font="Nanum Gothic Coding 12",padding=[5,5,5,5])
style.configure("Custom.TLabel",background="white",font="Nanum Gothic Coding 20",padding=[5,5,5,5])
style.configure("Custom.TButton",background="white",font="Nanum Gothic Coding 12 underline",padding=[5,5,5,5])
create_table()
show_translation_form(window)
